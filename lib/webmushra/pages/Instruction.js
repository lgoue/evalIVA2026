...
Instruction.prototype.render = function (_parent) {
  var div = $("<div></div>");
  var global = this;
  _parent.append(div);
  var content;
...
cb.append("&nbsp;&nbsp;"); // spacing
  ...
  for (let i = 0; i < n_choices; ++i) {
    ...
    choice.click(function(){
          ...
          if (global.questions[j].isAnswered() & good_responses[j] == i) {
            global.answer++;
            global.questions[j].setAnswered(true);
          }else if (!global.questions[j].isAnswered() & good_responses[j] != i){
            global.answer--;
            global.questions[j].setAnswered(false);
          }
          if (global.answer >= global.questions.length){
          global.pageTemplateRenderer.unlockNextButton();
          }else{
              global.pageTemplateRenderer.lockNextButton();
          }
...
Instruction.prototype.save = function () {
...
Instruction.prototype.postCheck = function() {
  var atKey = null;
  ...
  if (atKey){
    var atTarget = 0
    if (atKey.includes('No')){
      atTarget = 0;
    }else if (atKey.includes('Yes')){
      atTarget = 1;
    }
...
Instruction.prototype.load = function () {
...
Instruction.prototype.store = function () {
  var trial = new Trial();
...
Instruction.prototype.checkNumConditions = function (_conditions) {
