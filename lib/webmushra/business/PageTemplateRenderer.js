...
PageTemplateRenderer.prototype.renderNavigation = function(_parentId) {
  ...
  if (this.pageManager.getCurrentPage() instanceof VideoPC) {
    if (this.pageManager.getPageIndex() < (this.pageManager.getNumPages() - 1)) {
      ...
      var buttonReload = $("<button id='__button_reload' data-role='button' data-inline='true' onclick='" + this.pageManager.getPageManagerVariableName() + ".reloadVideo();'>" + this.pageManager.getLocalizer().getFragment(this.language, "reloadButton")  + "</button>");
      buttonReload.css('margin-right', '24em');
      $('#' + this.navigationId).append(buttonReload);
      renderedSomething = true;
    }
  }
